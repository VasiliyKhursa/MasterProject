...
{
	#ifdef UART_AS_OneWire													// передача осуществляется с помощью УАРТа
		UCSRB=(1<<RXEN)|(1<<TXEN);
		//9600
	...
	#else																	// передача осуществляется с помощью програмной реализации
...
//----------------------------функція запису байта--------------------------
	...
	void OW_WriteByte(unsigned char byte)									// передача осуществляется с помощью програмной реализации
		{
		for (unsigned char i=0; i<8; i++) OW_WriteBit(CheckBit(byte, i));
		}

//----------------------------функція читання байта--------------------------
	unsigned char OW_ReadByte(void)											// передача осуществляется с помощью програмной реализации
		{
		unsigned char n=0;
